...
class ApSolicitudes extends ActiveRecord {
    ...
    protected function initialize() {
        ...
        $this->validates_presence_of('nombreResponsable', 'message: Debe escribir un <b>Nombre</b> para el Responasble');
        $this->validates_presence_of('telefonoResponsable', 'message: Debe escribir un <b>Telefono</b> para el Responsable');
        $this->validates_presence_of('emailReponsable', 'message: Debe escribir un <b>Correo</b> para el Responsable');
        $this->validates_presence_of('nombreSolicitante', 'message: Debe escribir un <b>Nombre</b> para el Solicitante');
        $this->validates_presence_of('telefonoSolicitante', 'message: Debe escribir un <b>Telefono</b> para el Solicitante');
        $this->validates_presence_of('emailSolicitante', 'message: Debe escribir un <b>Correo</b> para el Solicitante');
        $this->validates_presence_of('apoyos_pkApoyo', 'message: Debe selecionar un <b>Apoyo</b> para la Solicitud');
        $this->validates_presence_of('nombreInstitucion', 'message: Debe escribir el <b>Nombre</b> de la Institución');
        $this->validates_presence_of('domicilio', 'message: Debe escribir el <b>Domicilio</b> de la Institución');
        $this->validates_presence_of('referenciaDomicilio', 'message: Debe escribir una <b>Referencia</b> de la Institución');
        $this->validates_presence_of('nivelEscolar', 'message: Debe escribir el <b>Nivel Escolar</b> de la Institución');
    ...
    public function guardarSolicitud($data, $data2, $data3) {
        ...
        //  print_r($solicitud_id);
        //      $solicitud->pkSolicitud = 233;
        ...
        if (!$solicitud->save()) {
            $solicitud->rollback();
            $solicitud->sql("ALTER TABLE ap_solicitudes auto_increment =$solicitud_id");
            return FALSE;
        } else {
        ...
        if ($data2['pkInstitucion'] == '') {
            ...
            $aist = $institucion->find($pkInstitucion);
        ...
        $ap_instituciones_has_solicitudes = new ApInstitucionesHasSolicitudes();
        if (!$ap_instituciones_has_solicitudes->guardarRelacion($pkSolicitud, $pkInstitucion)) {
            $solicitud->rollback();
            $solicitud->sql("ALTER TABLE ap_solicitudes auto_increment =$solicitud_id");
            return FALSE;
        }

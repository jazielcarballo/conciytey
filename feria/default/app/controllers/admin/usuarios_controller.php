    ...
    public function imagen() {
        ...
        try {
            if (Input::hasPost('oculto')) {
                ...
                $band = false;
                $archivo->setExtensions(array('png')); //le asignamos las extensiones a permitir
                // if ($archivo->validates()) {
                if ($archivo->isUploaded()) {
                    if ($archivo->save(Auth::get("login"))) {
                        Flash::valid('foto actualizada Exitosamente...!!!');
                        return Router::redirect("admin/usuarios/perfil");
                    }
                } else {
                    Flash::error('No se pudo cambiar la foto...!!!');
                    return Router::redirect('admin/usuarios/perfil');
                }
            } else {
